...
def main():
    ...
    # ── Пресинхронизация J2/J3 по первому зелёному J1 ────────
    # Момент, когда J1 впервые даст MAIN_GO = green_at (абс. секунда)
    j1_first_green = TL_CONFIGS["J1"]["green_at"]  # = 53 от t=0

    N2 = 0; N3 = 0  # очередей ещё нет — симуляция только стартовала
    d_accel      = (V_CONST * T_ACCEL) / 2
    t_reach_12   = T_ACCEL + (D_12 - N2 * DIST_CAR - d_accel) / V_CONST - N2 * T_DELAY
    j2_first_green = j1_first_green + t_reach_12

    t_reach_23   = (D_23 - N3 * DIST_CAR) / V_CONST - N3 * T_DELAY
    j3_first_green = j2_first_green + t_reach_23

    sync_green_at("J2", TL_CONFIGS["J2"], j2_first_green)
    sync_green_at("J3", TL_CONFIGS["J3"], j3_first_green)
    print(f"[INIT] J1 зелёный → {j1_first_green}s | J2 → {j2_first_green:.1f}s | J3 → {j3_first_green:.1f}s")
